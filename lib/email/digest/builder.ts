...
import { ads, audiences, campaigns, releases, artists, assets, adMetricDaily, releaseMetricDaily } from "@/lib/db/schema";
...
export async function buildCampaignDigest(args: {
  ...
  const [artist] = await db.select().from(artists).where(eq(artists.id, campaign.artistId)).limit(1);
  ...
  const [rm] = await db
    ...
    .limit(1);
  const baseline = await computeReleaseBaseline(release.id, campaign.startDate);
  ...
  const pendingRows = await db
    ...
    .where(and(eq(ads.campaignId, args.campaignId), eq(ads.status, "pending")));
  ...
  const pendingAds: PendingAdSummary[] = [];
  for (const { ad, audience } of pendingRows) {
